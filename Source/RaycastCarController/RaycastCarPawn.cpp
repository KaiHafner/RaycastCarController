...
ARaycastCarPawn::ARaycastCarPawn()
{
	...
	WheelOffsets.Add(FVector(50, 50, 0));   // Front-right
	WheelOffsets.Add(FVector(50, -50, 0));  // Front-left
	WheelOffsets.Add(FVector(-50, 50, 0));  // Rear-right
	WheelOffsets.Add(FVector(-50, -50, 0)); // Rear-left
}
...
void ARaycastCarPawn::ApplySuspensionForce(FVector Start, FVector End, float DeltaTime)
{
	...
	if (GetWorld()->LineTraceSingleByChannel(HitResult, Start, End, ECC_Visibility, Params))
	{
		...
		float CompressionRatio = 1.0f - (DistanceToGround / SuspensionMaxLength);
